    ...
    try {
      ...
      const response = await fetch(
        "/api/vereinsleitung/meetings/" + meetingId + "/protocol",
        {
          ...
          body: JSON.stringify({
            agendaItemTitle: agendaItemTitle.trim() || null,
          ...
          <div className="grid gap-4">
            <div>
              ...
              <input
                ...
                onChange={(event) => setAgendaItemTitle(event.target.value)}
                placeholder="z. B. Trainingsplan Winter"
      ...
      {protocolEntries.length > 0 ? (
        ...
        <div className="mt-6 rounded-[24px] border border-dashed border-slate-300 bg-slate-50 px-5 py-6">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-500">
            Dieser Bereich ist jetzt fuer strukturierte Protokolleintraege vorbereitet und wird im
            naechsten Schritt agenda-item-basiert ausgebaut.
          </p>
        </div>
      )}
    </section>
