...
async function findMeetingId(meetingIdOrSlug: string) {
...
export async function GET(
  ...
  const meeting = await prisma.vereinsleitungMeeting.findFirst({
    where: {
      OR: [{ id: params.meetingId }, { slug: params.meetingId }],
    },
    include: {
      matterLinks: {
...
export async function PATCH(
  ...
  try {
    const params = await context.params;
    const existingMeeting = await findMeetingId(params.meetingId);
    ...
    const participantCreateData: Prisma.VereinsleitungMeetingParticipantCreateWithoutMeetingInput[] =
    ...
    const updatedMeeting = await prisma.vereinsleitungMeeting.update({
      where: {
      ...
      data: {
        ...
        matterLinks:
          matterIdsResult.value !== undefined
            ? {
                deleteMany: {},
            ...
            : undefined,
        participants:
          participantsResult.value !== undefined
            ? {
                deleteMany: {},
