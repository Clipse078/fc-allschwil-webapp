/**
 * Tests for GET /api/cron/sfv-club-master-import
...
const mockRunAutomaticSfvClubMasterImport = vi.fn();

vi.mock("@/lib/integrations/sfv/sync/auto-club-master-import", () => ({
  runAutomaticSfvClubMasterImport: mockRunAutomaticSfvClubMasterImport,
}));
...
function makeRequest(headers: Record<string, string> = {}): NextRequest {
  return new NextRequest("http://x/api/cron/sfv-club-master-import", { method: "GET", headers });
}
...
describe("GET /api/cron/sfv-club-master-import — authorization", () => {
  it("rejects with 401 when CRON_SECRET is not configured (fail closed)", async () => {
    ...
    expect(mockRunAutomaticSfvClubMasterImport).not.toHaveBeenCalled();
  });
  ...
  it("accepts a request with the correct Authorization: Bearer <CRON_SECRET> header", async () => {
    process.env.CRON_SECRET = "test-cron-secret";
    mockRunAutomaticSfvClubMasterImport.mockResolvedValue({
    ...
    expect(mockRunAutomaticSfvClubMasterImport).toHaveBeenCalledOnce();
  });
...
describe("GET /api/cron/sfv-club-master-import — response", () => {
  beforeEach(() => {
    process.env.CRON_SECRET = "test-cron-secret";
  });
  ...
  it("returns the sanitized summary in the response body", async () => {
    const summary = {
      startedAt: "2026-08-08T04:00:00.000Z",
      finishedAt: "2026-08-08T04:00:02.000Z",
      durationMs: 2000,
      tenantsDiscovered: 2,
      tenantsSynced: 1,
      tenantsFailed: 1,
      tenants: [
        ...
        { tenantId: "tenant-b", outcome: "failed", code: "SfvAuthError", message: "boom" },
      ],
    };
    mockRunAutomaticSfvClubMasterImport.mockResolvedValue(summary);
  ...
  it("returns 500 without leaking internal error details when the orchestrator throws unexpectedly", async () => {
    mockRunAutomaticSfvClubMasterImport.mockRejectedValue(new Error("unexpected internal failure with details"));
