...
export default async function TargetsPage({ searchParams }: PageProps) {
  ...
  try {
    ...
    console.error("[targets] Failed to load targets:", message);
    loadError = message;
  }
        ...
        <>
          ...
          {!targets || targets.length === 0 ? (
            <section className="rounded-[30px] border border-slate-200/80 bg-white p-10 shadow-[0_10px_30px_rgba(15,23,42,0.04)] text-center">
              ...
              <Link
                href="/targets/new"
                className="mt-5 inline-flex items-center gap-2 rounded-full bg-[#3f63b5] px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-[#2f52a0]"
              >
                <Plus className="h-4 w-4" />
