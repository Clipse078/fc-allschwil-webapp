      ...
      <AdminSectionHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            <Link
              href={`/targets/${id}/edit`}
              className="inline-flex items-center gap-1.5 rounded-full bg-[#3f63b5] px-4 py-2 text-sm font-semibold text-white hover:bg-[#2f52a0]"
            >
        ...
        <div className="space-y-5">
          <section className="rounded-[28px] border border-slate-200/80 bg-white p-6 shadow-[0_10px_30px_rgba(15,23,42,0.04)]">
            <div className="mb-5 flex flex-wrap items-center gap-2">
              <span className={`rounded-full border px-3 py-1.5 text-[11px] font-semibold ${statusInfo.classes}`}>
                {statusInfo.label}
              </span>
                        ...
                        {metric.dataPoints.slice(0, 5).map((dp) => (
                          ...
                          >
                            <span className="text-[12px] font-semibold text-slate-900">
                              {dp.value}{metric.unit ? ` ${metric.unit}` : ""}
                            </span>
        ...
        <aside className="space-y-5">
          <section className="rounded-[28px] border border-slate-200/80 bg-white p-5 shadow-[0_10px_30px_rgba(15,23,42,0.04)]">
            ...
            <dl className="space-y-3">
              ...
              <div>
                ...
                <dd className="text-sm font-medium text-slate-900">{formatSwissDate(target.createdAt)}</dd>
              </div>
            </dl>
          </section>
          ...
          <section className="rounded-[28px] border border-slate-200/80 bg-white p-5 shadow-[0_10px_30px_rgba(15,23,42,0.04)]">
            <div className="mb-4 flex items-center gap-2">
              ...
              <h3 className="text-[13px] font-semibold uppercase tracking-[0.12em] text-slate-500">Governance</h3>
            </div>
            <div className="space-y-3">
              <div>
                <p className="mb-1.5 text-[11px] text-slate-400">Prüfstatus</p>
                <ReviewStageBadge stage={target.reviewStage} />
              </div>
              {target.reviewedAt ? (
                <div>
                  <p className="mb-0.5 text-[11px] text-slate-400">Geprüft am</p>
                  <p className="text-sm font-medium text-slate-900">{formatSwissDate(target.reviewedAt)}</p>
                </div>
              ) : null}
              <div>
                <p className="mb-2 text-[11px] text-slate-400">Statuswechsel</p>
                <TargetStageActions targetId={id} currentStage={target.reviewStage} />
              </div>
            </div>
          </section>
          ...
          {target.metrics.length > 0 ? (
            <section className="rounded-[28px] border border-slate-200/80 bg-white p-5 shadow-[0_10px_30px_rgba(15,23,42,0.04)]">
              <h3 className="mb-4 text-[13px] font-semibold uppercase tracking-[0.12em] text-slate-500">Fortschritt</h3>
