      ...
      <div className="grid gap-5 xl:grid-cols-[1fr_300px]">
        ...
        <aside className="space-y-5">
          <section className="rounded-[28px] border border-slate-200/80 bg-white p-5 shadow-[0_10px_30px_rgba(15,23,42,0.04)]">
            ...
            {usedKeys.length === 0 ? (
              <p className="text-[12px] text-slate-400 italic">Keine Variablen verwendet.</p>
            ) : (
                ...
                {usedKeys.map((k) => {
                  ...
                  return (
                    <div key={k} className="rounded-[12px] border border-slate-100 bg-slate-50 px-3 py-2">
                      ...
                      {def ? (
                        <p className="mt-0.5 text-[10px] text-slate-500">{def.label} · Bsp: {def.example}</p>
                      ) : (
                        <p className="mt-0.5 text-[10px] text-amber-600">Unbekannte Variable</p>
                      )}
                    </div>
                  );
                })}
