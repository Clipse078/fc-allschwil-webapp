      ...
      <AdminSectionHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            <Link href="/vereinsleitung/templates" className="sce-action-secondary px-4 py-2 text-sm font-medium">
              <ArrowLeft className="h-4 w-4" />Zurück
            </Link>
            <Link href={`/vereinsleitung/templates/${id}/edit`} className="sce-action-primary px-4 py-2 text-sm">
        ...
        <div className="space-y-5">
          <section className="sce-page-card p-6">
            <div className="mb-4 flex items-center gap-2">
              <Sparkles className="h-4 w-4 text-[var(--sce-primary-strong)]" />
              <h3 className="sce-section-title">Vorschau (Beispieldaten)</h3>
            </div>
            <p className="mb-1.5 text-[11px] font-semibold uppercase tracking-[0.1em] text-slate-400">Betreff</p>
            <p className="mb-5 rounded-[14px] border border-[var(--sce-border)] bg-[var(--sce-surface-muted)] px-4 py-3 text-sm font-medium text-[var(--sce-heading)]">{previewSubject}</p>
                ...
                {usedKeys.map((k) => {
                  ...
                  return (
                    <div key={k} className="rounded-[12px] border border-slate-100 bg-slate-50 px-3 py-2">
                      <p className="font-mono text-[11px] text-[var(--sce-primary-strong)]">{`{{${k}}}`}</p>
