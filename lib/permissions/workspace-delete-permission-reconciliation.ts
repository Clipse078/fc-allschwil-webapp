/**
 * lib/permissions/workspace-delete-permission-reconciliation.ts
 *
 * ADMIN-DELETE-03A — idempotent workspace.delete permission backfill.
 *
 ...
 * database, reused here for the workspace.delete permission rather than
 * reinvented.
 *
 * ── Why this exists ──────────────────────────────────────────────────────────
 * prisma/seed.ts is NOT automatically re-run after a deploy. Any STAGE/
 * production database seeded before the ADMIN-DELETE-03A commit added
 * workspace.delete to prisma/seed.ts is missing:
 ...
 *   - Permission row (module=WORKSPACE / scope=TENANT / grantableByAdmin=true —
 *     matches prisma/seed.ts exactly).
 *   - RolePermission: super_admin (PLATFORM role) → workspace.delete.
 ...
 *     from lib/roles/tenant-role-keys.ts) → workspace.delete.
 *
 ...
 * scripts/sync-workspace-delete-permission.ts wraps it for controlled,
...
export const WORKSPACE_DELETE_SUPER_ADMIN_ROLE_KEY = "super_admin";
export const WORKSPACE_DELETE_CLUB_ADMIN_KEY_PREFIX = `${CLUB_ADMIN_TEMPLATE_KEY}__`;

export const WORKSPACE_DELETE_PERMISSION_DEF = {
...
export type WorkspaceDeletePermissionReconciliationResult = {
...
/**
 * Reconciles the ADMIN-DELETE-03A workspace.delete permission and its bootstrap
 * role grants. Idempotent: safe to call multiple times without side effects.
 * Does NOT commit any changes when `dryRun` is true.
 */
export async function reconcileWorkspaceDeletePermission(
  prisma: PrismaClient,
  dryRun = false,
): Promise<WorkspaceDeletePermissionReconciliationResult> {
  const def = WORKSPACE_DELETE_PERMISSION_DEF;
  ...
  const superAdminOutcome = await assignPermissionToRole(
    prisma,
    WORKSPACE_DELETE_SUPER_ADMIN_ROLE_KEY,
    def.key,
    dryRun,
  );
  ...
  const tenantClubAdminRoles = await prisma.role.findMany({
    where: {
      scope: "TENANT",
      isSystem: true,
      key: { startsWith: WORKSPACE_DELETE_CLUB_ADMIN_KEY_PREFIX },
    },
    select: { key: true },
  });
