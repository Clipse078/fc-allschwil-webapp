 ...
 *
 * Covers (INFOBOARD-05 test requirements — facility data):
 ...
 *   8.  Current activity preferred over upcoming when both exist
 ...
 *   - dressingRooms is always empty (Screen 2 does not render cabin data)
...
function makeEvent(overrides: Partial<{
  ...
  return {
    ...
    seasonKey: "2025-26",
    team: { name: "FC Test", displayName: "FC Test", shortName: "FCT" },
    opponent: null,
    opponentFallbackName: "FC Opponent",
    organizerName: null,
...
// ── 8. Current activity preferred over upcoming ────────────────────────────────

describe("8. Current activity preferred over upcoming", () => {
  it("pitch with both current and upcoming events returns OCCUPIED_NOW (not UPCOMING)", async () => {
...
describe("dressingRooms", () => {
  it("dressingRooms is always empty in Screen 2 feed (INFOBOARD-05)", async () => {
    const startAt = new Date(NOW_UTC.getTime() - 10 * 60_000);
