...
export default function PasswordResetForm({ token }: PasswordResetFormProps) {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMessage(data?.message ?? "Passwort erfolgreich gesetzt.");
      setPassword("");
      setConfirmPassword("");
    } catch (submitError) {
      setError(
      ...
      <div className="relative z-10 flex min-h-screen items-center justify-center px-4 py-10">
        <div className="w-full max-w-[420px] rounded-[14px] border border-slate-200 bg-white/95 p-6 shadow-[0_18px_50px_rgba(15,23,42,0.10)] backdrop-blur">
          ...
          <div className="mb-7 text-center">
            <h1 className="text-[18px] font-semibold text-[#1f2937]">Passwort neu setzen</h1>
            <p className="mt-2 text-sm leading-6 text-slate-500">
              Ein Admin hat dir einen Reset-Link geschickt. Bitte setze jetzt dein neues Passwort.
            </p>
          </div>
