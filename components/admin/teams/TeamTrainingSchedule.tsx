...
export default function TeamTrainingSchedule({
  entries,
  ...
  return (
    <SectionCard
      title="Training"
      description="Trainingszeiten aus dem TrainingCenter."
      headerActions={
        trainingCenterHref ? (
          <Link
    ...
    >
      {entries.length === 0 ? (
        ...
        <div className="space-y-3" data-testid="team-training-schedule">
          {entries.map((entry) => (
            <div
              key={`${entry.seriesId}-${entry.weekday}`}
              className="flex items-start gap-3 rounded-lg border border-[var(--border)] bg-[var(--surface)] px-3.5 py-3"
            >
              <Clock className="mt-0.5 h-4 w-4 shrink-0 text-[var(--muted)]" />
              <div className="min-w-0">
                <p className="text-sm font-medium text-[var(--foreground)]">{entry.weekdayLabel}</p>
