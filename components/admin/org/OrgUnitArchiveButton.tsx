...
export default function OrgUnitArchiveButton({ orgUnitId, orgUnitName }: Props) {
  ...
  async function handleArchive() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/org-units/${orgUnitId}`, { method: "DELETE" });
  ...
  if (!confirming) {
    return (
      <div className="space-y-2">
        ...
        {error ? (
          <p className="text-center text-[11px] font-medium text-rose-600">{error}</p>
        ) : null}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3 rounded-[var(--radius-xl)] border border-rose-200 bg-rose-50 p-4">
      <p className="text-[13px] font-semibold text-rose-800">
        &bdquo;{orgUnitName}&ldquo; wirklich archivieren?
      </p>
      <p className="text-[12px] text-rose-700">
        Archivierte Einheiten sind nicht mehr aktiv. Mitgliedschaften bleiben erhalten.
        Diese Aktion kann durch Bearbeiten der Einheit rückgängig gemacht werden.
      </p>
