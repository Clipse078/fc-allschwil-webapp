...
/**
 * WaitingListCoordinatorPicker — REG-WAIT-01D / REG-WAIT-01E / REG-WAIT-01F
 *
...
export function WaitingListCoordinatorPicker({
  ...
  const filtered = useMemo(
    () => filterCoordinatorsBySearch(eligibleCoordinators, query),
    [eligibleCoordinators, query],
  );

  const selectedLabel = selected ? `${selected.firstName} ${selected.lastName}` : null;
  const showEmptyState = !selected && query.trim().length >= 2 && filtered.length === 0;
    ...
    <div className="w-full">
      {selected ? (
        ...
        <div ref={anchorRef} className="relative w-full">
          ...
          >
            <UserCheck
              className={cn("flex-shrink-0 text-[var(--muted)]", compact ? "h-3.5 w-3.5" : "h-4 w-4")}
              aria-hidden
            />
            <input
              type="text"
            ...
            onChange={(e) => {
              ...
              onKeyDown={(e) => {
                ...
                if (e.key === "Enter" && listActiveIndex >= 0 && filtered[listActiveIndex]) {
                  e.preventDefault();
                  handleSelect(filtered[listActiveIndex].id);
                }
              }}
          ...
          {showEmptyState ? (
            <p className="mt-1 text-xs italic text-[var(--muted)]">Keine passende Person gefunden.</p>
          ) : null}
                ...
                return (
                  <li key={user.id} role="option" aria-selected={index === listActiveIndex}>
                    <button
                      ...
                      className={cn(
                        "flex w-full items-center gap-3 px-3 py-2.5 text-left transition-colors hover:bg-[var(--surface-2)]",
  ...
  return (
    <div ref={anchorRef} className="relative">
      ...
      >
        ...
        <span className="max-w-[120px] truncate">{displayLabel ?? "Verantwortlich"}</span>
        {value ? (
          <span
            role="button"
            tabIndex={0}
