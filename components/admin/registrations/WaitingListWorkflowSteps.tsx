          ...
          {steps.map((step, index) => {
            ...
            return (
              <li key={step.key} className="relative flex flex-1 min-w-0 flex-col items-center text-center">
                ...
                {index < steps.length - 1 ? (
                  <span
                    ...
                    className={cn(
                      ...
                      index < currentIndex || entry.status === "PLACED"
                ...
                >
                  {isCompleted && !isCurrent ? <CheckCircle2 className="h-3.5 w-3.5" aria-hidden /> : index + 1}
                </div>
                ...
                {step.timestamp ? (
                  <p className="mt-0.5 px-1 text-[0.62rem] text-[var(--muted)]">
                    {formatWaitingListDateTime(step.timestamp)}
                  </p>
                ) : null}
              </li>
            );
        ...
        <div className="mt-4 flex items-start gap-2 rounded-[var(--radius-md)] border border-rose-200 bg-rose-50 px-3 py-2.5">
          ...
          <div>
            <p className="text-xs font-semibold text-rose-800">
              Abgeschlossen: {WAITING_LIST_STATUS_LABELS[terminalStatus]}
            </p>
            {entry.resolvedAt ? (
