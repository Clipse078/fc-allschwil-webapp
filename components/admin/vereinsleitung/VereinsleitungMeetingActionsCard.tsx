          ...
          <div className="mt-6 divide-y divide-slate-100 overflow-hidden rounded-[22px] border border-slate-200/80 bg-white shadow-[0_6px_18px_rgba(15,23,42,0.03)]">
            {actions.map((a) => (
              <div key={a.id} className="flex items-start gap-4 px-4 py-4">
                <div className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-md border ${a.status === "DONE" ? "border-[#3f63b5] bg-[#3f63b5] text-white" : "border-slate-300 bg-white text-slate-400"}`}>
      ...
      <div className="mt-6 divide-y divide-slate-100 overflow-hidden rounded-[22px] border border-slate-200/80 bg-white shadow-[0_6px_18px_rgba(15,23,42,0.03)]">
        {MOCK_ACTIONS.map((a) => (
          <div key={a.title} className="flex items-start gap-4 px-4 py-4 transition hover:bg-slate-50/70">
            <button type="button" className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-md border ${a.completed ? "border-[#3f63b5] bg-[#3f63b5] text-white" : "border-slate-300 bg-white text-slate-400"}`}>
              <Circle className="h-3.5 w-3.5" />
            </button>
            <div className="min-w-0 flex-1">
            ...
            <div className="flex shrink-0 items-center gap-3">
              <span className="flex h-6 w-6 items-center justify-center rounded-full bg-[#3f63b5]/10 text-[10px] font-semibold text-[#3f63b5]">{a.ownerInitials}</span>
