        ...
        <div>
          ...
          {!hasRealData ? (
            <p className="mt-0.5 text-[11px] text-slate-400">
              Demo-Daten ·{" "}
              <Link href="/vereinsleitung/targets" className="text-[#3f63b5] hover:underline">
        ...
        {displayItems.map((item) => (
          <Link key={item.id} href={item.href} className="block space-y-2.5 transition hover:opacity-80">
            ...
            <div className="h-2 rounded-full bg-slate-100">
              <div
                className={`h-2 rounded-full transition-all ${item.progress >= 90 ? "bg-emerald-500" : item.progress >= 60 ? "bg-[#3f63b5]" : item.progress >= 30 ? "bg-amber-500" : "bg-rose-400"}`}
