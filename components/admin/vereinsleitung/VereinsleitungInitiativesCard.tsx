              ...
              <Link key={ini.id} href={`/vereinsleitung/initiativen/${ini.slug}`} className="block space-y-3">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex min-w-0 items-start gap-3">
                    <div className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-[12px] bg-[var(--sce-primary-soft)]">
                      <TrendingUp className="h-4 w-4 text-[var(--sce-primary)]" />
                    </div>
                  ...
                  <div className="pl-11">
                    <div className="flex items-center justify-between gap-2 mb-1">
                      <div className="h-1.5 flex-1 rounded-full bg-slate-100">
                        <div className="h-1.5 rounded-full bg-[var(--sce-primary)]" style={{ width: `${Math.min(100, ini.progress)}%` }} />
                      </div>
          ...
          : MOCK_INITIATIVES.map((ini) => (
              <div key={ini.title} className="space-y-3">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex min-w-0 items-start gap-3">
                    <div className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-[12px] bg-[var(--sce-primary-soft)]">
                      <Flag className="h-4 w-4 text-[var(--sce-primary)]" />
                    </div>
                    <div className="min-w-0">
                      <p className="truncate text-[14px] font-semibold text-slate-900">{ini.title}</p>
                ...
                <div className="pl-11">
                  <div className="flex items-center gap-2">
                    <div className="h-1.5 flex-1 rounded-full bg-slate-100">
                      <div className="h-1.5 rounded-full bg-[var(--sce-primary)]" style={{ width: `${ini.progress}%` }} />
                    </div>
                    <span className="shrink-0 text-[11px] font-semibold text-slate-500">{ini.progress}%</span>
                  </div>
                </div>
