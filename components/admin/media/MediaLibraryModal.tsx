...
export default function MediaLibraryModal({
  ...
  // Sync pending selection with the incoming selectedId when the modal opens
  useEffect(() => {
    if (isOpen) {
  ...
  // Load assets when modal opens or filter changes
  useEffect(() => {
      ...
      >
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-3 border-b border-[var(--border)] px-6 py-3">
          ...
          <div className="inline-flex rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--surface-2)] p-0.5 text-xs font-medium">
            {(["ALL", "IMAGE", "VIDEO"] as FilterType[]).map((f) => (
              <button
                ...
                type="button"
                onClick={() => setFilter(f)}
                className={`inline-flex items-center gap-1 rounded-[var(--radius-sm)] px-3 py-1.5 transition-all ${
                  filter === f
          ...
          <button
            ...
            disabled={uploading}
            className="fca-button-secondary gap-1.5 text-xs"
          >
            {uploading ? (
          ...
          <button
            ...
            onClick={loadAssets}
            disabled={loading}
            className="sce-icon-button h-9 w-9"
            title="Aktualisieren"
          >
        ...
        {/* ── Upload error ── */}
        {uploadError && (
          <div className="shrink-0 border-b border-rose-200 bg-rose-50 px-6 py-2 text-xs font-medium text-rose-700">
            {uploadError}
          </div>
        )}

        {/* ── Main body: grid + optional selected panel ── */}
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5">
            {error ? (
              <div className="flex flex-col items-center gap-3 py-20 text-[var(--muted)]">
                ...
                <button type="button" onClick={loadAssets} className="fca-button-secondary text-xs">
                  Erneut versuchen
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4">
                {Array.from({ length: 12 }).map((_, i) => (
                  <div
                    key={i}
                    className="aspect-[4/3] animate-pulse rounded-[var(--radius-xl)] bg-[var(--surface-2)]"
                  />
                ))}
              </div>
        ...
        <div className="flex shrink-0 items-center justify-between gap-4 border-t border-[var(--border)] bg-[var(--surface-2)] px-6 py-4">
          <p className="text-[11px] text-[var(--muted)]">
            {!loading && visibleAssets.length > 0
              ? `${visibleAssets.length} von ${total} Medien`
              : ""}
          </p>
          <div className="flex items-center gap-3">
...
type ModalAssetCardProps = {
  asset: MediaAssetListItem;
  isSelected: boolean;
  onSelect: () => void;
};

function ModalAssetCard({ asset, isSelected, onSelect }: ModalAssetCardProps) {
  return (
    <button
      type="button"
      onClick={onSelect}
      ...
      <div className="flex flex-col gap-0.5 p-3">
        <p
          className={`truncate text-xs font-medium ${
            isSelected ? "text-[var(--tenant-primary)]" : "text-[var(--foreground)]"
          }`}
...
function SelectedAssetPanel({ asset, onClear }: SelectedAssetPanelProps) {
  return (
    <div className="flex flex-col gap-0 p-4">
      ...
      {/* Details */}
      <p className="mb-1 text-[11px] font-semibold uppercase tracking-[0.08em] text-[var(--muted)]">
        Ausgewählt
      </p>
      <div className="mb-4 space-y-2.5 rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface)] p-3">
        ...
        {asset.width && asset.height ? (
          <DataRow label="Auflösung" value={`${asset.width} × ${asset.height}`} />
        ) : null}
        {asset.mimeType ? (
          <DataRow label="Format" value={asset.mimeType.split("/")[1]?.toUpperCase() ?? asset.mimeType} />
        ) : null}
      </div>
      ...
      <button
        type="button"
        onClick={onClear}
        className="fca-button-secondary w-full text-xs text-rose-600"
      >
        <X className="h-3.5 w-3.5" />
        Auswahl aufheben
      </button>
...
function DataRow({
  ...
  return (
    <div className="flex flex-col gap-0.5">
      <span className="text-[9px] font-semibold uppercase tracking-[0.1em] text-[var(--muted)]">
        {label}
      </span>
      <span
        className={`text-xs font-medium text-[var(--foreground)] ${truncate ? "truncate" : "break-all"}`}
