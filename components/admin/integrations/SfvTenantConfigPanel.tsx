 ...
 *
 * CLUB-DIRECTORY-05-C1: the SFV club master (Vereinsverzeichnis) import has
 * no manual trigger here — tenant admins must not be able to run it on
 * demand. It runs automatically once per day via cron
 * (app/api/cron/sfv-club-master-import/route.ts →
 * runAutomaticSfvClubMasterImport() → the same runSfvClubMasterImport()
 * service CLUB-DIRECTORY-05 introduced). This panel only displays the
 * read-only "Letzte Vereinsverzeichnis-Synchronisierung" timestamp for
 * observability, sourced from TenantSfvConfig.lastClubMasterImportAt.
 *
 * Security:
