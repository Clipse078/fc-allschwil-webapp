...
// ── Tournament Sync (diagnostic-only) ─────────────────────────────────────────

function makeTournamentSyncResult(overrides: Record<string, unknown> = {}) {
  return {
    startedAt: "2026-08-06T10:00:00.000Z",
    finishedAt: "2026-08-06T10:00:00.050Z",
    durationMs: 50,
    tenantId: TENANT_ID,
    source: "SFV",
    clubId: 483,
    ...
    warnings: [
      {
        ...
        message: "Die SFV Club API stellt keinen strukturierten Endpunkt für Turniere bereit.",
      },
    ],
    recommendedAction:
...
describe("Tournament sync (diagnostic-only)", () => {
  it("button disabled when not configured", () => {
    render(<SfvTenantConfigPanel initialConfig={null} />);

    expect(screen.getByTestId("btn-tournament-sync")).toBeDisabled();
  });

  it("button enabled when configured", () => {
    render(<SfvTenantConfigPanel initialConfig={makeConfig()} />);

    expect(screen.getByTestId("btn-tournament-sync")).not.toBeDisabled();
  });

  it("shows blocked diagnostic result with warning and recommendation after clicking sync", async () => {
    globalThis.fetch = mockFetchSuccess({ result: makeTournamentSyncResult() });

    render(<SfvTenantConfigPanel initialConfig={makeConfig()} />);

    fireEvent.click(screen.getByTestId("btn-tournament-sync"));

    await waitFor(() => {
      expect(screen.getByTestId("tournament-sync-result")).toBeTruthy();
    });

    expect(screen.getByTestId("tournament-sync-status").textContent).toContain(
      "Keine Anbieterquelle verfügbar",
    );
    expect(screen.getByTestId("tournament-sync-warnings").textContent).toContain(
      "PROVIDER_SOURCE_UNAVAILABLE",
    );
    expect(screen.getByTestId("tournament-sync-recommendation").textContent).toContain(
      "Turnier erstellen",
    );
  });

  it("all counts render as zero in the blocked diagnostic result", async () => {
    globalThis.fetch = mockFetchSuccess({ result: makeTournamentSyncResult() });
    ...
    const result = screen.getByTestId("tournament-sync-result");
    expect(result.textContent).toContain("Fehler");
    expect(result.textContent).not.toContain("undefined");
  });

  it("shows 404 error when no SFV configuration exists", async () => {
    globalThis.fetch = mockFetchError(404, { error: "No SFV configuration found for this tenant" });
    ...
    await waitFor(() => {
      expect(screen.getByTestId("tournament-sync-error")).toBeTruthy();
    });

    expect(screen.getByTestId("tournament-sync-error").textContent).toContain(
      "Bitte zuerst speichern",
    );
  });

  it("shows 409 error when SFV integration is disabled", async () => {
    globalThis.fetch = mockFetchError(409, { error: "SFV integration is disabled" });
    ...
    expect(screen.getByTestId("tournament-sync-error").textContent).toContain("deaktiviert");
  });
  ...
  it("shows network error message on fetch rejection", async () => {
    ...
    expect(screen.getByTestId("tournament-sync-error").textContent).toContain("Netzwerkfehler");
  });

  it("repeated clicks (idempotent diagnostic) render the same blocked result each time", async () => {
    globalThis.fetch = vi
      .fn()
      .mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: () => Promise.resolve({ result: makeTournamentSyncResult() }),
      })
      .mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: () =>
          Promise.resolve({
            result: makeTournamentSyncResult({
              startedAt: "2026-08-06T11:00:00.000Z",
              finishedAt: "2026-08-06T11:00:00.050Z",
            }),
          }),
      });
    ...
    const firstRunText = screen.getByTestId("tournament-sync-result").textContent;
    ...
    const secondRunText = screen.getByTestId("tournament-sync-result").textContent;

    // Both runs are blocked with identical counts/warnings/recommendation and
    // duration — startedAt/finishedAt are the only fields that would differ,
    // and neither is rendered in the UI.
    expect(firstRunText).toBe(secondRunText);
  });
});