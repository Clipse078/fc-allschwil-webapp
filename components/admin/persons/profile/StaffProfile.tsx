...
import { CareerTimelinePlaceholder, ProfileCard, ProfileEmptyState, ProfileKpiTile, ProfileShell, formatDate } from "./PersonProfileLayout";
  ...
  return (
    <ProfileShell person={person} name={name} primaryType={primaryType} typeLabels={typeLabels}>
      <div className="grid gap-4 md:grid-cols-3">
        <ProfileKpiTile icon={<Clock3 className="h-5 w-5" />} label="Offene Aufgaben" value={String(person.vereinsleitungOwnedMatters.length)} />
        <ProfileKpiTile icon={<ShieldCheck className="h-5 w-5" />} label="Rollen" value={String(roleNames.length)} />
        <ProfileKpiTile icon={<CheckCircle2 className="h-5 w-5" />} label="Aktiv" value={person.isActive ? "Ja" : "Nein"} highlight />
      </div>
