      ...
      <div className="space-y-3">
        {value ? (
          <div className="relative overflow-hidden rounded-[var(--radius-xl)] border border-[var(--border)]">
            <img src={value.url} alt={value.altText ?? value.filename} className="h-48 w-full object-cover" />
            <div className="absolute inset-x-0 bottom-0 flex items-center justify-between bg-black/60 px-3 py-2">
              ...
              <button type="button" onClick={() => onChange(null)}
                className="ml-2 rounded-full bg-white/20 p-1 text-white transition hover:bg-white/40" title="Headerbild entfernen">
                <X className="h-3 w-3" />
              </button>
            </div>
          </div>
        ...
        <div className="flex flex-wrap gap-2">
          <button type="button" onClick={() => setModalOpen(true)} className="fca-button-secondary text-xs">
            <FolderOpen className="h-3.5 w-3.5" />
            {value ? "Bild wechseln" : "Aus Bibliothek wählen"}
          </button>
          {value && (
            <button type="button" onClick={() => onChange(null)} className="fca-button-secondary text-xs text-rose-600">
