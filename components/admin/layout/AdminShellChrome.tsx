 ...
 *
 * Renders the full admin chrome (glows, watermark, sidebar, stage banner,
 * impersonation bar, top header, main content area).
 *
 * Consumed by app/(admin)/layout.tsx after session is resolved.
 * All visual output is identical to the previously inlined layout code.
 */
                ...
                <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.18em] text-amber-700">
                      {t.auth.impersonationActive}
                    </p>
                    <p className="mt-1 text-sm text-amber-900">
                      {t.auth.impersonationDescription}
                    </p>
                    <p className="mt-1 text-xs text-amber-800">
                      Admin: {actorName ?? actorEmail ?? t.auth.unknownAdmin}
                    </p>
                  </div>
